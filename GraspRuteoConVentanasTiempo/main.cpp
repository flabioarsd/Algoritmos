...
void mostrarSolucion(vector<Auto>autos){
    for(Auto &au : autos){
        ...
        for(Cliente &cli : au.listaAtendidos){
            cout<<"Codigo de cliente: "<<cli.id_cliente<<"     "<<endl;
            if(cli.emHora)cout<<" Atendido en hora"<<endl;
            else cout<<"Atendido fuera de hora"<<endl;
        }
    ...
    vector<Auto> resultado;
        ...
        while(!it_clietes.empty()){
           ...
           // imprimeAutos(it_autos);
            ...
            /* Condicion de salida auxiliar*/
            if(noHayEspacios(it_autos,it_clietes[c_indrcl].cantCajas))break;
...
void graspRutaMinima(Auto &autos,vector<vector<int>>mapa){
    //para seleccionar los vecinos usa dijktra
    Auto mejorRespuesta;
    ...
    while(not autos.listaPendientes.empty() and horadeldia<=autos.horaSalida){
        ...
        distanciaRecorrida=vecinos[indrcl].distancia;
...
void lectura(vector<Auto>&autos,vector<Cliente>&clientes,vector<vector<int>>&mapa){
    ...
    while(true){
        ...
        archAutos>>car>>pos>>car>>velocidad>>car>>maxCapacidad>>car>>hIng>>car>>hSal;
    ...
    while(true){
        ...
        archMapa>>car>>b>>car>>c>>car>>d>>car>>e>>car>>f;
        line.push_back(a);
        line.push_back(b);
        line.push_back(c);
        line.push_back(d);
